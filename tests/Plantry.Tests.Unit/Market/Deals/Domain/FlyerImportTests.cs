...
public sealed class FlyerImportTests
{
    ...
    [Fact(DisplayName = "RecordRepull on a Parsed import refreshes content hash + window (DD5/DD13)")]
    public void RecordRepull_RefreshesBookkeeping_AndReemits()
    {
        ...
        var result = import.RecordRepull([1, 2, 3], newWindow, clock.Advance(TimeSpan.FromDays(1)));
    ...
    public void RecordRepull_RequiresParsed()
    {
        ...
        var result = import.RecordRepull([9], Window(), new TestClock());
