...
public sealed class IngestFlyerTests
{
    ...
    private sealed class Harness
    {
        ...
        public FakePriceObservationRepository Observations { get; } = new();
        public TestClock Clock { get; } = new();
        public FakeTenantContext Tenant { get; } = new(Household.Value);

        public IngestFlyer Build()
        {
            var confirm = new ConfirmDeal(
                Deals, Memories, Products, Observations, new FakeUnitPriceCalculator(1m), Clock, Tenant,
                NullLogger<ConfirmDeal>.Instance, NullLogger<RecordObservationCommand>.Instance);
    ...
    public async Task Materializes_Remembered_And_Pending()
    {
        ...
        var obs = Assert.Single(h.Observations.Items);
    ...
    public async Task NoTenant_IsEmptyNoOp()
    {
        ...
        var confirm = new ConfirmDeal(
            h.Deals, h.Memories, h.Products, h.Observations, new FakeUnitPriceCalculator(1m), h.Clock,
            new FakeTenantContext(null), NullLogger<ConfirmDeal>.Instance, NullLogger<RecordObservationCommand>.Instance);
