...
// FakePriceObservationRepository and FakeUnitPriceCalculator now live in
// tests/Plantry.Tests.Unit/Market/PriceObservationFakes.cs (Plantry.Tests.Unit.Market) — shared with the
// Prices test tree since both write/read the same price_observation table (ADR-024 merge). Consumed here
// via the `using Plantry.Tests.Unit.Market;` above.
