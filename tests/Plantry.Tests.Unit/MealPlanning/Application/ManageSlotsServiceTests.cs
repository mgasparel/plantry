...
public sealed class ManageSlotsServiceTests
{
    ...
    [Fact]
    public async Task ListMembersAsync_Delegates_To_Reader()
    {
        var repo = new FakeMealSlotConfigRepository(null);
        var member = new HouseholdMember(Guid.NewGuid(), "Alice", "A");
        var reader = new FakeSlotsMemberReader(member);
        var svc = BuildService(repo, reader);

        var result = await svc.ListMembersAsync();
...
public sealed class FakeSlotsMemberReader(params HouseholdMember[] members) : IHouseholdMemberReader
{
