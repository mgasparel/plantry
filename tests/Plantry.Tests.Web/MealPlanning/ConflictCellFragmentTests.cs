...
public sealed class ConflictCellFragmentTests(ConflictCellFactory factory)
{
    ...
    [Fact(DisplayName = "POST Generate: irreconcilable hard-stance → grid renders conflict-notice with advisory text")]
    public async Task PostGenerate_HardConflict_RendersConflictNoticeText()
    {
        ...
        Assert.Contains("No single dish suits everyone here", html);
    }
