...
public sealed class TwoProposalRegenerateFactory : MealPlanFragmentFactory
{
    protected override IRecipeReadModel RecipeReadModel => new TwoProposalRecipeReader();

    // Mutable and returned as a single shared instance (the property is evaluated once and handed to
    // AddSingleton), so state persists across requests within this factory.
    protected override IPendingProposalStore ProposalStore => new TwoProposalPendingStore();
}
...
/// </summary>
public sealed class RegenerateCellFactory : MealPlanFragmentFactory
{
