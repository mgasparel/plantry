...
public static class ShoppingListFixture
{
    ...
    /// Milk: 2 L on-hand (not low). Flour: 0 g on-hand (out; IsLow = false — out is not "running
    /// low", per plantry-43y). This exercises both the "N unit in pantry" and "out" render paths.
    /// </summary>
    public static IReadOnlyDictionary<Guid, ShoppingPantryStockLevel> StockLevels() =>
        new Dictionary<Guid, ShoppingPantryStockLevel>
        {
            ...
            [FlourProductId] = new(FlourProductId, OnHand: 0m,   UnitCode: "g",  IsLow: false),
        };

    /// <summary>
    /// Unit options for the fixture (plantry-259) — used to populate add-form and inline qty editor.
    /// Deliberately left in non-dimension-sorted order (Volume before Mass) — both the add-form's
    /// <c>UnitOptions</c> and the inline per-row editor's <c>InlineUnitOptions</c> re-sort via
    /// <c>UnitSelectListBuilder.Build</c> (grouped plantry-n9iw / plantry-pe7n respectively), so this
    /// also proves that re-sort happens rather than merely preserving whatever order the reader returns.
    /// </summary>
