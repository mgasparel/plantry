...
**Status:** Accepted · 2026-06-06 · Amended 2026-06-06, 2026-07-25 (plantry-kw52)
...
- **Recipes** — `Recipe` root with `Ingredient` children (`product_id`, qty, unit, group heading, order) and directions. Fulfillment % and cost/serving are **read models**, not aggregates — composed at query time from Recipe × ProductStock × PriceObservation. Cook is an application service: scale by servings, apply transient swap/skip choices, issue consume commands to Inventory. *(Amended 2026-07-25, plantry-kw52 — see amendment below: the cook anchor commit may persist a just-in-time `Recipe.SetYield` mutation in the SAME transaction as the `CookEvent` anchor, a named bounded exception to one-per-transaction.)*
...
## Amendment · 2026-07-25 — Cook-anchor Recipe-yield co-commit is a bounded exception (plantry-kw52)

Code review of the just-in-time yield resolution (plantry-iejb) surfaced a design question: `CookRecipe` (`src/Plantry.Recipes/Application/CookRecipe.cs`) calls `recipe.SetYield(...)` to record a newly-picked or newly-created yield product on the `Recipe` aggregate, then later calls `cookEvents.SaveChangesAsync(ct)` to persist the `CookEvent` anchor. Because `IRecipeRepository` and `ICookEventRepository` share one scoped `RecipesDbContext`, that single `SaveChangesAsync` flushes the tracked `Recipe` mutation **and** the new `CookEvent` in one transaction — a multi-aggregate co-commit the §Aggregates "one-per-transaction" rule doesn't, on its face, sanction. This was **ratified with Michael (2026-07-24)** as a named bounded exception, not a violation to fix:

- **The cook-anchor commit may persist the just-in-time `Recipe.SetYield` mutation in the SAME transaction as the `CookEvent` anchor.** `Recipe` and `CookEvent` are both Recipes-context aggregates sharing one `RecipesDbContext`/connection, exactly the "one bounded context, one scoped connection" precondition the 2026-07-11 Identity amendment above used to justify user-creation + invite-acceptance atomicity — this is the same shape, not a new kind of exception. No invariant spans `Recipe` and `CookEvent`, so nothing is lost by co-committing them; atomicity is actively *desirable*: splitting `SetYield` into its own transaction **before** the anchor would let a yield-product mapping persist for a cook that then fails to anchor (a remembered yield for a cook that never happened); splitting it **after** would let the anchor commit while a separate `SetYield` save fails (the cook happened but the yield is forgotten). The single flush guarantees the remembered yield and the cook are durable together or not at all. `SetYield` is NOT split into its own transaction or context — that was explicitly considered and rejected.
- **This does not reopen Gate 2's "each downstream call is its own transaction" clause.** That phrase (`.claude/review-criteria.md` Gate 2) governs the fan-out of Inventory/Shopping calls that run **after** the anchor commits — each of those remains its own transaction, driven by the application service, never folded into the anchor. The Recipe-yield co-commit sits entirely **inside** the anchor commit itself (before any downstream call runs), so the two statements describe different phases of the same flow and are not in tension.
- **Scope is narrow and named, not a general license.** This exception covers exactly one call pair — `Recipe.SetYield` co-committing with the `CookEvent` anchor inside `CookRecipe` — because both aggregates already share the Recipes context. It does not extend to any other cross-aggregate save, in Recipes or elsewhere; a new case needs its own amendment.

## Amendment · 2026-08-03 — Context map consolidated: ten contexts fold to six (ADR-024)

With the app fleshed out, a boundary review found the context *count* — not the discipline — generating most of the friction: ~60 translation adapters in `Plantry.Composition`, per-context duplicate DTOs for identical reads, page models composing six contexts, and micro-contexts (Pricing, Housekeeping) paying a full boundary tax while protecting no invariant. **[ADR-024](ADR-024.md) supersedes this ADR's nine-context map**: Catalog + Inventory → **Pantry**, Pricing + Deals → **Market**, Meal Planning + Shopping → **Planning**, Housekeeping dissolves into the composition read layer; Recipes, Intake, and Identity & Access are unchanged. Everything else in this ADR carries forward against the six-context map — aggregate one-per-transaction rules, ID-only references, the ACL stance, and the named exceptions above. Merging contexts removes translation ceremony, not consistency rules.
