...
| DM-22 | Deals (Phase 5): four **flat** aggregates — `store_subscription` root (`UNIQUE(household_id, store_id)`), `flyer_import` root (the ACL provenance envelope: `raw_flyer` jsonb quarantine, `status` `text`+`CHECK`, `content_hash` bytea, `UNIQUE(household_id, store_id, flyer_external_id)` dedup), `deal` root (its **own** long-lived root, not a `flyer_import` child — `flyer_import_id` within-context composite FK RESTRICT/nullable for the deferred manual path; `status`/`match_confidence`/`source` `text`+`CHECK`; the `suggested_*` ACL quarantine vs the resolved `product_id`; `valid_from`/`valid_to`; `committed_price_observation_id` soft-ref), and `deal_match_memory` root (`UNIQUE(household_id, store_id, normalized_name)`, nullable `product_id` for negative memory, `normalizer_version` + retained `raw_name` for normalizer-change backfill). Persisted ACL **mirrors intake's `import_session`** (DM-15), the **opposite** of Meal Planning's transient store (DM-21): flyer review is async. Confirm projects exactly one `price_observation` (`source=deal`, the seam Pricing pre-built — DM-17); a Correct supersedes append-only. `ActiveDeal`/`StockUpAlert` are **computed read-side, no tables**; `IActiveDealReader` is exposed to Shopping for the deal badge. `catalog.store` (DM-16) lands this phase. Amends ADR-010 Deals aggregates (`Store`→`StoreSubscription`; adds `FlyerImport`). | ✅ |
