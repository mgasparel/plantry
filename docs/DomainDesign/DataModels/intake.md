...
> **ADR note (ADR-010, reconciled):** merchant identity belongs in **Catalog** (a small `store` reference aggregate), not Deals; Deals (Phase 3) keeps its flyer-source *config* aggregate referencing `catalog.store` by ID. **Finalized (DM-16, [pricing.md](pricing.md)):** Phase-1 Intake stays on free-text `merchant_text`, which `price_observation` carries alongside a nullable `store_id` soft-ref; the `store` table is deferred to Phase 3.
