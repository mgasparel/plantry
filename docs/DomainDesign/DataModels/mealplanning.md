...
> **Feeds the next step.** The §8 ports — `IRecipeReadModel`, `ITagReader`, `ICatalogProductReader`, `IInventoryStockReader`, `IPriceReader`, `IShoppingListWriter`, and `IHouseholdMemberReader` — become application-service interfaces in the App Services pass. Recipe IDs are selected by `DeterministicWeekMealOptimizer` in trusted Planning code.
