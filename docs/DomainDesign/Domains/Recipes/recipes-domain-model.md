...
| **CostingService** (domain) | `Compute(recipe, servings) → CostPerServing`. Counts costable ingredients (tracked, real product), sums those with price history, and sets `Completeness` (`Full`/`Partial`/`None`) + the missing-price list accordingly. | `IPriceReader`, `IUnitConverter` |
...
|---|---|---|
| **ICatalogProductReader** | Author, Cook, Fulfillment | R: product name, `track_stock`, `default_unit_id`, parent/variant tree (DM-10, DM-19) |
...
| **IPriceReader** | Costing | R: latest/representative `PriceObservation` per product (DM-17) |
