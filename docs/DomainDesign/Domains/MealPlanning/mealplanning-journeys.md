...
| C6 | Irreconcilable hard preferences at a shared meal | **P3 DETECTS & FLAGS (not auto-split).** When attendees' hard stances conflict (one `Vegan`-`Required`, one meat-`Required`) and no single candidate recipe satisfies both, the planner **leaves the cell unfilled and marks it in-cell** during generate/review. Two distinct cell states exist: **HardConflict** (C6 — attendees clash, recipes exist for each individually) renders "Attendees' requirements conflict — no single dish works for everyone" + dual CTAs ("Add a dish by hand" / "Adjust who's attending"); **Unfulfillable** (so5.5 — an attendee's Required tag has zero recipes anywhere in the corpus) renders "Your recipe book has no X recipes" + "Add a X recipe" CTA. HardConflict takes precedence if both conditions apply. Neither is a rail insight (the `HardConflictResolved` insight was removed in so5.4). **Generative per-attendee auto-split is deferred to FUTURE.** |
...
| C15 | Plan insights (advisory callouts) | A computed, **advisory** insights panel visible **during review and on a saved plan** (J10): e.g. **`UnusedExpiring`** ("Chicken breast expires in 3 days, but isn't used in this plan"), **`OverBudget`**, **`Repetition`** (same recipe this week / repeated from last week), **`UnfilledSlot`**. Read-side only — observations, never blocks (C12) — computed over the plan + Inventory expiring stock + Recipes reads; **no new ports**. Note: a hard-stance conflict (C6) is an **unfillable-cell state** (rendered in-cell, not as a rail insight) — see so5.4 / so5.5. |
...
| 2 | System | Gathers context **server-side**: pantry + expiring stock (Inventory); each active **plated** candidate recipe's tags + live fulfillment + cost (Recipes read models); the configured slots, their **effective attendees**, and **those attendees' stances** (C5). Unchecked recipes remain available to manual selection and inclusion expansion. |
...
| 5 | System | **Pre-flight checks — two reasons a cell is skipped without calling the AI:** (a) **HardConflict** (`HardConflictDetector`): no single **plated** candidate recipe from the household's recipe corpus satisfies all attendees' hard stances simultaneously (C6 — checked first). (b) **Unfulfillable** (`UnfulfillabilityDetector`): an attendee's Required tag has **zero** matching **plated** recipes anywhere in the household's full active corpus — the corpus itself cannot satisfy the need. Both are flagged in-cell with distinct, actionable UI (HardConflict: "requirements conflict" + dual CTAs; Unfulfillable: "recipe book has no X recipes" + "Add a X recipe" CTA). HardConflict takes precedence if a cell satisfies both conditions. Cells that pass both checks proceed to the AI planner. **ACL validation** then covers satisfiable cells: rejects malformed payloads, drops dishes with Restricted tags, enforces per-attendee Required satisfaction; impossible cells → unfilled. Raw payload is **not persisted** (C10). No per-attendee auto-split in P3. |
...
| 2 | System | Surfaces advisory callouts, e.g.: **`UnusedExpiring`** ("Chicken breast expires in 3 days, but isn't used in this plan"); **`OverBudget`** ("Est. $96 vs $80 target"); **`Repetition`** ("Tacos planned twice this week"; "Same as last Tuesday"); **`UnfilledSlot`** ("No dinner planned Thu–Fri"). Note: a hard-stance conflict (C6) is an **in-cell marker** on the conflicted cell ("No single dish suits everyone here"), not a rail callout — `HardConflictResolved` has been removed from `InsightKind` (so5.4). |
...
**Attendance is the join between slots and preferences.** A meal's constraint set is the union of its **effective attendees'** stances — not the whole household's. This is what lets "lunch is planned for me alone" and "dinner must satisfy both of us" coexist. In P3, multi-dish meals support main+side combinations; when hard stances conflict (C6), the cell is detected as irreconcilable and flagged in-cell — per-attendee generative splitting is deferred to FUTURE (C5/C6).
