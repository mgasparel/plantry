...
| C6 | Irreconcilable hard preferences at a shared meal | **P3 DETECTS & FLAGS (not auto-split).** When attendees' hard stances conflict (one `Vegan`-`Required`, one meat-`Required`) and no single candidate recipe satisfies both, the planner **leaves the cell unfilled and marks it in-cell** during generate/review — a distinct cell state ("No single dish suits everyone here"), not a rail insight. The user acts on the cell (add a compromise dish by hand, adjust attendance, eat separately). **Generative per-attendee auto-split with serving apportionment is deferred to FUTURE** because of the serving/waste cascade (two dishes each defaulting to full servings → ~2× food → leftover/waste, cascading into shopping list and cost). so5.4 delivers the in-cell seed; so5.5 builds the full actionable empty-cell UI (same shape as the Unfulfillable state). |
...
| 5 | System | **Pre-flight conflict detection:** runs `HardConflictDetector` per cell before dispatching to the planner. Irreconcilable cells (C6: no single candidate satisfies every attendee) are **flagged in-cell** ("No single dish suits everyone here") and skipped from the AI request — no auto-split in P3. **ACL validation** then covers satisfiable cells: rejects malformed payloads, drops dishes with Restricted tags, enforces per-attendee Required satisfaction; impossible cells → unfilled. Raw payload is **not persisted** (C10). |
