...
**Deal-awareness is a Phase-4 seam (C7).** SPEC §5b lists "checks active deals" as a generation input and VISION calls the planner deal-aware. With Deals deferred to Phase 4, Phase 3 ships the planner deal-blind; the cost input is purchase-price history only, and adding deal pricing later is an additional `IPriceReader`-style input, not a re-model.
