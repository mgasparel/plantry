...
| **HardConflictDetector** (domain) | `Detect(constraints, candidates) → HardStanceConflict?`. Pure, stateless. Determines whether no single candidate recipe can satisfy all attendees' hard stances simultaneously (C6). Returns null when reconcilable; conflicted cells are skipped from deterministic selection, counted as unfilled, and flagged in-cell. No I/O. | in-context |
...
| **GeneratePlan** (application) | J4. Takes a planning scope and constraints; runs conflict/unfulfillability checks, then deterministic server-owned week selection. Every selection is validated through `ProposalAcl` and written to the **session-keyed pending store** (§6). Weights tune soft scoring only and never relax a hard stance. | `IRecipeReadModel`, `ITagReader`, `IInventoryStockReader`, in-context resolver |
...
- **MP-O4 — Where attendee aggregation lives ✅.** In a **domain service**
  ...
  deterministic selection is given those constraints and remains in trusted code (M5).
...
wired in the App Services step; recipe identity is selected by `DeterministicWeekMealOptimizer`.
