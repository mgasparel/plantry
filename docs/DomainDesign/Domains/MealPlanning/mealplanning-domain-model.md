...
| **GeneratePlan** (application) | J4. Takes a **`PlanningScope`** (week / day / slot-series / single meal, C13) + `PlanningConstraints` (incl. **`PlanningWeights`** Cost/Waste/Variety and the optional budget target, C14); targets only **empty** cells in scope unless `replace = true`. For each cell: runs `HardConflictDetector`; irreconcilable cells (C6) are left unfilled and flagged in-cell — no per-attendee auto-split in P3 (deferred to FUTURE). For satisfiable cells: gathers context (candidate recipes + constraints), passes the weighted soft objective to `IMealPlanner` (untrusted, ADR-007), validates the raw output via `ProposalAcl` (per-attendee hard stance enforcement), and writes validated `ProposedMeal`s to the **session-keyed pending store** (§6). Weights tune the soft objective only; they never relax a hard stance (M5/M11). | `IRecipeReadModel`, `IInventoryStockReader`, `IMealPlanner`, in-context resolver |
...
| **M5** | An **AI-proposed** meal satisfies every effective attendee's hard stances: no dish carries a `Restricted` tag; every attendee's `Required` tags are all met by ≥ 1 surviving dish. If no single candidate satisfies all attendees simultaneously (C6), the cell is left **unfilled and flagged in-cell** — no per-attendee auto-split in P3 (deferred to FUTURE). **Manual** assignment **warns, never blocks** (C9). | C6, C9 | `HardConflictDetector` + `ProposalAcl` (hard) / `AssignMeal` (warn) |
