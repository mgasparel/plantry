...
| ~~D4~~ | ~~Is a "stocktake" a persisted record?~~ | **Resolved → C7.** No persisted session and no resume. Pending counts are page-only until Save; only the `Correction` journal rows persist. A per-product "Last counted N days ago" surface, if wanted, derives from the newest `source_type = Manual` `Correction` row — no new schema. |
