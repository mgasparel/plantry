...
| T4 | **Findings are computed live at page render — no persisted findings table.** **REVISED (ADR-024 Phase A, plantry-g3da.2):** the standalone Housekeeping bounded context described below was dissolved. The `Finding` read model, the `IProblemDetector` contract, the `Dismissal` aggregate + repository, and `GetTidyUpPageQuery` now live directly in **`src/Plantry.Web/Housekeeping/`** (Application/Domain/Persistence subfolders) — the composition root, which already legitimately references every context. The 7 detector implementations live in **`src/Plantry.Web/Housekeeping/Detectors/`**, and read other contexts' data through the shared ADR-021 cross-schema read models in **`src/Plantry.Web/Housekeeping/ReadModels/`** (`IStockFactsReadModel`/`IRecipeFactsReadModel`) rather than through per-context application services or read ports. Original decision (superseded): a standalone module `src/Plantry.Housekeeping` owned the read model/contract/aggregate/query service, with detector implementations in `src/Plantry.Composition/Housekeeping/`, reading other contexts only through their existing application services / read ports | Live computation avoids a sync problem (a persisted finding going stale the moment the user fixes the gap elsewhere). Detectors are inherently cross-context read compositions. The original Composition-seam placement (mirroring `ShoppingPantryReaderAdapter`) was replaced once ADR-021's shared read-model pattern made per-context read ports unnecessary for Tidy Up's household-wide scans — collapsing the bounded context into Plantry.Web removed a layer with no remaining boundary to police |
...
| D8 | Product without a default location | Active tracked non-parent product whose `default_location_id` is null, including catalog-only products without stock roots | New stock entries have no product-specific location prefilled; existing lots may still be stored in a physical location | Catalog product detail (`/Catalog/Products/{productId}`) | **shipped (plantry-f0od)** · A |

D8 deliberately checks the catalog product default only. Lot locations never infer or rewrite that default, and products with a non-null (including dangling) default are outside this detector.
