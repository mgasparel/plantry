// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using Plantry.Pantry.Infrastructure;

#nullable disable

namespace Plantry.Pantry.Infrastructure.Migrations.Pantry
{
    [DbContext(typeof(PantryDbContext))]
    [Migration("20260813053740_AddDefaultProducedCategory")]
    partial class AddDefaultProducedCategory
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasDefaultSchema("catalog")
                .HasAnnotation("ProductVersion", "10.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Plantry.Pantry.Domain.Category", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset?>("ArchivedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("archived_at");

                    b.Property<int?>("DefaultDueDays")
                        .HasColumnType("integer")
                        .HasColumnName("default_due_days");

                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<int?>("Hue")
                        .HasColumnType("integer")
                        .HasColumnName("hue");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("name");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer")
                        .HasColumnName("sort_order");

                    b.HasKey("Id");

                    b.HasIndex("HouseholdId", "Name")
                        .IsUnique();

                    b.ToTable("categories", "catalog");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.HouseholdInventorySettings", b =>
                {
                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<Guid?>("DefaultLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("default_location_id");

                    b.Property<Guid?>("DefaultProducedCategoryId")
                        .HasColumnType("uuid")
                        .HasColumnName("default_produced_category_id");

                    b.Property<int>("ExpiringSoonDays")
                        .HasColumnType("integer")
                        .HasColumnName("expiring_soon_days");

                    b.HasKey("HouseholdId");

                    b.ToTable("household_inventory_settings", "inventory");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.Location", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset?>("ArchivedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("archived_at");

                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<DateTimeOffset?>("LastCountedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_counted_at");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("name");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("location_type");

                    b.HasKey("Id");

                    b.HasIndex("HouseholdId", "Name")
                        .IsUnique();

                    b.ToTable("locations", "catalog");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.Product", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset?>("ArchivedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("archived_at");

                    b.Property<Guid?>("CategoryId")
                        .HasColumnType("uuid")
                        .HasColumnName("category_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<int?>("DefaultDueDays")
                        .HasColumnType("integer")
                        .HasColumnName("default_due_days");

                    b.Property<int?>("DefaultDueDaysAfterFreezing")
                        .HasColumnType("integer")
                        .HasColumnName("default_due_days_after_freezing");

                    b.Property<int?>("DefaultDueDaysAfterOpening")
                        .HasColumnType("integer")
                        .HasColumnName("default_due_days_after_opening");

                    b.Property<int?>("DefaultDueDaysAfterThawing")
                        .HasColumnType("integer")
                        .HasColumnName("default_due_days_after_thawing");

                    b.Property<Guid?>("DefaultLocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("default_location_id");

                    b.Property<Guid>("DefaultUnitId")
                        .HasColumnType("uuid")
                        .HasColumnName("default_unit_id");

                    b.Property<bool>("HasVariants")
                        .HasColumnType("boolean")
                        .HasColumnName("has_variants");

                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<bool>("IsProduced")
                        .HasColumnType("boolean")
                        .HasColumnName("is_produced");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("name");

                    b.Property<bool?>("NeverExpiresAfterFreezing")
                        .HasColumnType("boolean")
                        .HasColumnName("never_expires_after_freezing");

                    b.Property<bool?>("NeverExpiresAfterThawing")
                        .HasColumnType("boolean")
                        .HasColumnName("never_expires_after_thawing");

                    b.Property<Guid?>("ParentProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("parent_product_id");

                    b.Property<bool>("TrackStock")
                        .HasColumnType("boolean")
                        .HasColumnName("track_stock");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("HouseholdId");

                    b.HasIndex("HouseholdId", "Name")
                        .IsUnique();

                    b.ToTable("products", "catalog");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.ProductConversion", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<decimal>("Factor")
                        .HasPrecision(18, 6)
                        .HasColumnType("numeric(18,6)")
                        .HasColumnName("factor");

                    b.Property<Guid>("FromUnitId")
                        .HasColumnType("uuid")
                        .HasColumnName("from_unit_id");

                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_id");

                    b.Property<string>("Source")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("source");

                    b.Property<Guid>("ToUnitId")
                        .HasColumnType("uuid")
                        .HasColumnName("to_unit_id");

                    b.HasKey("Id");

                    b.HasIndex("HouseholdId");

                    b.HasIndex("ProductId");

                    b.ToTable("product_conversions", "catalog");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.ProductSku", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<string>("Label")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("label");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_id");

                    b.Property<decimal?>("SizeQuantity")
                        .HasPrecision(12, 3)
                        .HasColumnType("numeric(12,3)")
                        .HasColumnName("size_quantity");

                    b.Property<Guid?>("SizeUnitId")
                        .HasColumnType("uuid")
                        .HasColumnName("size_unit_id");

                    b.HasKey("Id");

                    b.HasIndex("HouseholdId");

                    b.HasIndex("ProductId");

                    b.ToTable("product_skus", "catalog");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.ProductStock", b =>
                {
                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<decimal?>("LowStockThreshold")
                        .HasPrecision(12, 3)
                        .HasColumnType("numeric(12,3)")
                        .HasColumnName("low_stock_threshold");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<uint>("xmin")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.HasKey("HouseholdId", "ProductId");

                    b.ToTable("product_stock", "inventory");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.StockEntry", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("entry_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTimeOffset?>("DepletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("depleted_at");

                    b.Property<DateOnly?>("ExpiryDate")
                        .HasColumnType("date")
                        .HasColumnName("expiry_date");

                    b.Property<DateTimeOffset?>("FrozenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("frozen_at");

                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<bool>("IsOpen")
                        .HasColumnType("boolean")
                        .HasColumnName("is_open");

                    b.Property<Guid>("LocationId")
                        .HasColumnType("uuid")
                        .HasColumnName("location_id");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_id");

                    b.Property<DateOnly?>("PurchasedAt")
                        .HasColumnType("date")
                        .HasColumnName("purchased_at");

                    b.Property<decimal>("Quantity")
                        .HasPrecision(12, 3)
                        .HasColumnType("numeric(12,3)")
                        .HasColumnName("quantity");

                    b.Property<Guid?>("SkuId")
                        .HasColumnType("uuid")
                        .HasColumnName("sku_id");

                    b.Property<DateTimeOffset?>("ThawedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("thawed_at");

                    b.Property<Guid>("UnitId")
                        .HasColumnType("uuid")
                        .HasColumnName("unit_id");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("HouseholdId", "LocationId", "ProductId")
                        .HasDatabaseName("ix_stock_entry_by_location")
                        .HasFilter("depleted_at IS NULL");

                    b.HasIndex("HouseholdId", "ProductId", "ExpiryDate", "CreatedAt")
                        .HasDatabaseName("ix_stock_entry_fefo");

                    b.ToTable("stock_entry", "inventory");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.StockJournalEntry", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("journal_id");

                    b.Property<decimal>("Delta")
                        .HasPrecision(12, 3)
                        .HasColumnType("numeric(12,3)")
                        .HasColumnName("delta");

                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<DateTimeOffset>("OccurredAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("occurred_at");

                    b.Property<Guid>("ProductId")
                        .HasColumnType("uuid")
                        .HasColumnName("product_id");

                    b.Property<string>("Reason")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("reason");

                    b.Property<Guid?>("SourceLineRef")
                        .HasColumnType("uuid")
                        .HasColumnName("source_line_ref");

                    b.Property<Guid?>("SourceRef")
                        .HasColumnType("uuid")
                        .HasColumnName("source_ref");

                    b.Property<string>("SourceType")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("source_type");

                    b.Property<Guid>("StockEntryId")
                        .HasColumnType("uuid")
                        .HasColumnName("entry_id");

                    b.Property<Guid>("UnitId")
                        .HasColumnType("uuid")
                        .HasColumnName("unit_id");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id");

                    b.HasIndex("HouseholdId");

                    b.HasIndex("StockEntryId");

                    b.HasIndex("HouseholdId", "ProductId");

                    b.HasIndex("HouseholdId", "SourceRef", "SourceLineRef")
                        .HasDatabaseName("ix_stock_journal_idempotency");

                    b.ToTable("stock_journal_entry", "inventory");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.Store", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset?>("ArchivedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("archived_at");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("ExternalRef")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("external_ref");

                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("name");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("Id");

                    b.HasIndex("HouseholdId", "ExternalRef")
                        .IsUnique()
                        .HasFilter("external_ref IS NOT NULL");

                    b.HasIndex("HouseholdId", "Name")
                        .IsUnique();

                    b.ToTable("stores", "catalog");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.Unit", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("symbol");

                    b.Property<string>("Dimension")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)")
                        .HasColumnName("dimension");

                    b.Property<string>("DisplayStyle")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("display_style");

                    b.Property<decimal>("FactorToBase")
                        .HasColumnType("numeric")
                        .HasColumnName("factor_to_base");

                    b.Property<Guid>("HouseholdId")
                        .HasColumnType("uuid")
                        .HasColumnName("household_id");

                    b.Property<bool>("IsBase")
                        .HasColumnType("boolean")
                        .HasColumnName("is_base");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("name");

                    b.Property<string>("UnitSystem")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("unit_system");

                    b.HasKey("Id");

                    b.HasIndex("HouseholdId", "Code")
                        .IsUnique();

                    b.ToTable("units", "catalog");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.ProductConversion", b =>
                {
                    b.HasOne("Plantry.Pantry.Domain.Product", null)
                        .WithMany("Conversions")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.ProductSku", b =>
                {
                    b.HasOne("Plantry.Pantry.Domain.Product", null)
                        .WithMany("Skus")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.StockEntry", b =>
                {
                    b.HasOne("Plantry.Pantry.Domain.ProductStock", null)
                        .WithMany("Entries")
                        .HasForeignKey("HouseholdId", "ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.StockJournalEntry", b =>
                {
                    b.HasOne("Plantry.Pantry.Domain.StockEntry", null)
                        .WithMany()
                        .HasForeignKey("StockEntryId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("Plantry.Pantry.Domain.ProductStock", null)
                        .WithMany("Journal")
                        .HasForeignKey("HouseholdId", "ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.Product", b =>
                {
                    b.Navigation("Conversions");

                    b.Navigation("Skus");
                });

            modelBuilder.Entity("Plantry.Pantry.Domain.ProductStock", b =>
                {
                    b.Navigation("Entries");

                    b.Navigation("Journal");
                });
#pragma warning restore 612, 618
        }
    }
}
