...
/// <see cref="MarketDbContext"/>'s household query filter, so the <c>(store, normalized_name)</c> lookup
/// resolves only within the current household (the third component of the DD3 uniqueness key).
/// </summary>
public sealed class DealMatchMemoryRepository(MarketDbContext db) : IDealMatchMemoryRepository
{
