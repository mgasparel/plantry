...
/// <c>SourceRef</c> is the recipe id. Manual contributions resolve to a fixed label; MealPlan resolves
/// via <see cref="IMealPlanRepository.FindSlotLabelsAsync"/> (an intra-context call since the Planning
/// merge, ADR-024) and Deal resolves via its own sibling port
