...
/// are resolved via <see cref="IMealPlanRepository.FindSlotLabelsAsync"/> — an intra-context call since
/// the MealPlanning/Shopping merge into Plantry.Planning (ADR-024, plantry-g3da.5; formerly the
/// Shopping→Meal Planning ACL port IShoppingMealPlanReader) — and
    ...
    ITenantContext tenant)
{
    ...
    public async Task<ShoppingListView?> GetListAsync(CancellationToken ct = default)
    {
        ...
        var mealPlanSlots = mealPlanSourceRefs.Count > 0
            ? await mealPlans.FindSlotLabelsAsync(mealPlanSourceRefs, ct)
            : (IReadOnlyDictionary<Guid, PlannedMealSlotInfo>)new Dictionary<Guid, PlannedMealSlotInfo>();
    ...
    private static ShoppingListItemView MapItem(
        ...
        IReadOnlyDictionary<Guid, PlannedMealSlotInfo> mealPlanSlots,
