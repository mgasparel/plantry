    ...
    public partial class RelabelPackAndDozenUnitReferences : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(dozPkCte +
                "UPDATE recipes.recipe r SET yield_unit_id = e.ea_id " +
                "FROM doz_pk d JOIN ea_units e ON e.household_id = d.household_id " +
                "WHERE r.yield_unit_id = d.id;");

            migrationBuilder.Sql(dozPkCte +
                "UPDATE recipes.recipe_ingredient ri SET unit_id = e.ea_id " +
                "FROM doz_pk d JOIN ea_units e ON e.household_id = d.household_id " +
                "WHERE ri.unit_id = d.id;");

            migrationBuilder.Sql(dozPkCte +
                "UPDATE recipes.cook_consume_line ccl SET unit_id = e.ea_id " +
                "FROM doz_pk d JOIN ea_units e ON e.household_id = d.household_id " +
                "WHERE ccl.unit_id = d.id;");

            migrationBuilder.Sql(dozPkCte +
                "UPDATE recipes.cook_produce_line cpl SET unit_id = e.ea_id " +
                "FROM doz_pk d JOIN ea_units e ON e.household_id = d.household_id " +
                "WHERE cpl.unit_id = d.id;");
        }
