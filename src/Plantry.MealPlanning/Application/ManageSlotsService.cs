    ...
    IClock clock)
{
    ...
    /// <summary>Returns the household members from the identity port.</summary>
    public Task<IReadOnlyList<HouseholdMember>> GetMembersAsync(Guid householdId, CancellationToken ct = default)
        => memberReader.GetMembersAsync(householdId, ct);
