...
/// model. <c>Plantry.Recipes.Domain.Ingredient.ProductId</c> is a non-nullable <see cref="Guid"/>
...
/// <para>
/// Fingerprint is the line's product id alone: re-pointing the same line at a different untracked
/// product is a new gap and must reopen a dismissed finding; the product later becoming tracked simply
/// stops the finding from firing (the detector's own condition handles that, not the fingerprint).
/// </para>
/// <para>
/// ADR-021/ADR-024 Phase A: loads its facts via <see cref="IRecipeFactsReadModel"/> (shared with D2/D5)
/// rather than the retired <c>IRecipeRepository</c>/<c>ICatalogProductReader</c> ports — the logic below
/// is unchanged from the original port-backed version.
/// </para>
/// </summary>
