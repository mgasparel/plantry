...
/// with the <b>effective (deal-aware) price</b> for a product by delegating to
/// <see cref="PricingQueries.EffectivePriceAsync"/>. Lives in Plantry.Web, the composition root that
/// already references the Pricing context, so the Recipes projects stay <c>→ SharedKernel only</c>.
///
/// Deal-aware costing (P5-9b, DJ6): <see cref="PricingQueries.EffectivePriceAsync"/> returns the cheapest
/// active in-window deal when one exists, else the latest purchase (ADR-010: the effective-price read model
/// lives in Pricing over <c>price_observation</c>; Recipes never learns about Deals). The deal window is
/// evaluated against <see cref="IClock"/>-derived "today", so a deal silently stops affecting cost once its
/// window lapses — the price is computed per query, never stored.
///
